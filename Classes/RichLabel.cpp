...
{
	CCDirector::sharedDirector()->getTouchDispatcher()->addTargetedDelegate(this, /*kCCMenuHandlerPriority - 20*/PRI, false);
}
...
int RichLabel::SearchStrPos(const char *string, int pixelWidth)
{
    std::string str;
    int result = 0;
    int len = strlen(string);
    ...
    for (int i = 1; i<=len;i++ ) {
        ...
        if (str.c_str()[i-1] < 0 || str.c_str()[i-1] > 127) {
            nPos++;
            if (nPos%3) {
                continue;
            }
...
void RichLabel::createLabel(const char * string, int length, bool defaultFont)
{
	...
	if (str.length())
	{
		...
		{
			//label->release();
			int widthd = (int)(mSize.width - mLabelOffsetX);
			//int pos = searchTruncatePos(str.c_str(), width);
            int pos = SearchStrPos(str.c_str(), widthd);
		...
		else // 单行即可完全显示
		{
			...
			else
			{
                ...
                //label->setPosition(ccp(mLabelOffsetX + getAnchorPoint().x * mSize.width,
                                       //+(mSize.width - label->getContentSize().width)/2,
                                       //mLabelOffsetY + getAnchorPoint().y * label->getContentSize().height));
