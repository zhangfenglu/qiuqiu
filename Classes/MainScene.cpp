...
//    this->scheduleUpdate();
    ...
    m_grid_off_pos = ccp(winSize.width * 0.5f, winSize.height * 0.5f);
    initJson();
...
void MainScene::CheckLayer(MainScene::tagDif tag)
{
    ...
    else if(tag == TAG_LAYER_USERINFO)
    {
        if(m_CurrTag == TAG_LAYER_LOGIN)
            ...
            CCNode* node = getChildByTag(m_CurrTag);
            node->setScale(0.0f);
        }
...
void MainScene::ccTouchesBegan(cocos2d::CCSet *pTouch, cocos2d::CCEvent *pEvent)
{
...
void MainScene::ccTouchesMoved(cocos2d::CCSet *pTouch, cocos2d::CCEvent *pEvent)
{
    if(m_isUpdateScale)
        return;
    ...
    diff = ccp((int)diff.x, (int)diff.y);
   
        // 滑杆移动
    SceneLayer* scene = dynamic_cast<SceneLayer*>(getChildByTag(TAG_LAYER_SCENE));
    if(scene)
    {
        scene->updateJoystickPos(diff);
        
        CCPoint tempDiff = scene->GetJoyDiff();
...
void MainScene::ccTouchesEnded(cocos2d::CCSet *pTouch, cocos2d::CCEvent *pEvent)
{
    // 现在希望不拖动也能移动
//    m_diff = ccp(0,0);
    ...
    if(layer)
    {
        CCPoint tempDiff = layer->GetJoyDiff();
...
void MainScene::update(float ft)
{
    ...
    CCPoint diff = m_diff * speed * m_scale;
    
    // 判断边界, 现在服务器做，暂时注掉
    //diff = calculateBorder(diff, size);
    diff = ccp((int)diff.x, (int)diff.y);
    
    //CCLog("diff x is:%f, y is:%f", diff.x, diff.y);
    
    // 客户端先模拟移动
    //    GridLayer* layer = dynamic_cast<GridLayer*>(getChildByTag(TAG_LAYER_GRID));
    //    CCPoint currentPos = layer->getPosition();
    ...
    // 通知服务器移动
    // 4-16 zyd修改球中心点
    CCPoint mainOldPos = Global::getInstance()->GetMainPos();
    //    GridLayer* grid = dynamic_cast<GridLayer*>(getChildByTag(TAG_LAYER_GRID));
    //    CCPoint mainOldPos = grid->getMainNodesCenterPos();
    CCPoint mainDest = ccpAdd(mainOldPos, diff / m_scale);
    //CCLog("old pos x is:%f, old pos y is:%f", mainOldPos.x, mainOldPos.y);
    MoveTo msg;
    msg.msgid = 16;
    ...
    WebSocketLayer* socket = dynamic_cast<WebSocketLayer*>(getParent()->getChildByTag(1));
    socket->onSendMsg((char*)&msg, sizeof(MoveTo));
    //CCLog("main:(x = %f , y = %f)", mainDest.x, mainDest.y);
    //m_diff = m_diff / (speed * m_scale);
    
#endif
}

CCPoint MainScene::calculateBorder(cocos2d::CCPoint pos, uint32_t size)
{
    CCPoint diff = pos;
    CCPoint MainPos = Global::getInstance()->GetMainPos();
    CCPoint wantPos = ccpAdd(MainPos, pos);
...
void MainScene::setWinSize(const WebSocket::Data &data)
{
    //设置SceneLayer
    setTouchEnabled(true);
    ...
    CCPoint pos = ccpAdd(ccp(m_left, m_top), m_grid_off_pos);
    CCSize size = CCSizeMake(m_right - m_left, m_bottom - m_top);
    ...
    if(grid == NULL)
    {
        CCLayerColor* color = CCLayerColor::create(ccc4(237, 242, 221, 255));
        ...
        //CCLog("grid posx:%f, posy: %f, width is: %f, height is: %f, left_time is:%d", grid->getPositionX(), grid->getPositionY(), size.width, size.height, m_leftTime);
    }
...
void MainScene::addSelfNode(const WebSocket::Data &data)
{
    uint32_t nodeid;
    int headSize = sizeof(uint8_t);
...
void MainScene::UpdateScaleByMainNodes()
{
    ...
    {
        //CCLog("scene scale is:%f, minScale is:%f", m_sceneScale, m_minScale);
...
void MainScene::updateNodes(const WebSocket::Data &data)
{
    GridLayer* grid = dynamic_cast<GridLayer*>(getChildByTag(TAG_LAYER_GRID));
    if(grid)
    {
...
void MainScene::updateMainState(CCPoint mainPos)
{
...
//    if(!m_isJoyMove && m_isFirstEnter)
    {
        ...
        if(grid != NULL)
        {
            CCPoint currentPos = grid->getPosition();
            CCPoint mainOldPos = Global::getInstance()->GetMainPos();
            CCPoint mainRelaPos = ccpSub(mainPos, mainOldPos);
            CCPoint destPos = ccpSub(currentPos, mainRelaPos * m_scale); // 有缩放的过程
            Global::getInstance()->SetMainPos(mainPos);
            Global::getInstance()->SetGridPos(destPos);
            grid->setPosition(destPos);
            //CCLog("return posx is:%f, posy is:%f", destPos.x, destPos.y);
...
int MainScene::getMainNodesSize()
{
    ...
    //CCLog("the main node size is:%d", size);
    return size;
}
...
void MainScene::updateSceneSize(float siz)
{
    ...
    GridLayer* grid = dynamic_cast<GridLayer*>(getChildByTag(TAG_LAYER_GRID));
//    CCPoint mainPos = Global::getInstance()->GetMainPos();
    CCPoint mainPos = grid->getMainNodesCenterPos();
    CCPoint dest = ccpSub(m_grid_off_pos * siz, mainPos * siz);
    //CCLog("dest x is::%f, dest y is:%f", dest.x, dest.y);
...
//    CCSize gridSize = Global::getInstance()->GetGridSize();
//    CCPoint anchor = ccp(mainPos.x / gridSize.width, mainPos.y / gridSize.height);
...
//    
//    CCLog("main nodes size is:%d", size);
...
void MainScene::UpdateScaleImmediately(float diff)
{
    ...
    //size = oldSize;
    CCPoint oldPos = grid->getPosition();
    CCPoint mainPos = grid->getMainNodesCenterPos();
    CCPoint dest = ccpSub(m_grid_off_pos * size, mainPos * size);
    float sizeDiff = size-oldSize;
    if( (sizeDiff < 0 && sizeDiff < -NEED_TO_PER_CAMERA) || (sizeDiff > 0 && sizeDiff > NEED_TO_PER_CAMERA) )
    {
        CCPoint diffPos = ccpSub(dest, oldPos) / NEED_TO_PER_CAMERA_TIMES;
        float diff = sizeDiff / NEED_TO_PER_CAMERA_TIMES;
        //CCLog("diff size is:%f, diff posx is:%f, diff posy is:%f", diff, diffPos.x, diffPos.y);
        for(int i = 1 ; i <= NEED_TO_PER_CAMERA_TIMES; i++ )
        {
            CCPoint midPos = oldPos + diffPos * i;
...
void MainScene::updateRankeInFight(const WebSocket::Data &data)
{
    int headSize = sizeof(uint8_t);
...
void MainScene::GameOverState(const WebSocket::Data &data)
{
    int headSize = sizeof(uint8_t);
    
    uint16_t count;
    memcpy(&count, data.bytes + headSize, sizeof(uint16_t));
    //CCLog("over state count is:%d", count);
    headSize += sizeof(uint16_t);
...
void MainScene::UpdateFightLife(const WebSocket::Data &data)
{
    int headSize = sizeof(uint8_t);
...
void MainScene::SetDeath(const WebSocket::Data &data)
{
    int headSize = sizeof(uint8_t);
...
void MainScene::loginGame()
{
    ...
    CCLog("user: %s   %s",UserAcc.c_str(),UserPass.c_str());
    // ui login
    UM_Login req;
...
void MainScene::reqSetFocus(int type, int set, int roleID)
{
    ...
    //CCLog("type is:%d, set is:%d, the roleID is:%d", type, set, roleID);
    std::string str = req.SerializeAsString();
...
{
    CCLog("picdatalen:%d",dataStr.length());
    UM_SetPhoto req;
...
// s->c
void MainScene::setPlayerInfo(const WebSocket::Data &data)
{
    UM_EnterGame resp;
...
void MainScene::respSyncRole(const WebSocket::Data &data)
{
    UM_SyncRole resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
void MainScene::respServerList(const WebSocket::Data &data)
{
    UM_ServerList resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len -2);
    if(re)
...
void MainScene::respServerKey(const WebSocket::Data &data)
{
...
void MainScene::respErrors(const WebSocket::Data &data)
{
    UM_Response resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
    {
        ...
        switch (err) {
            case SERR_OK:
                //CCLog("response id is:%d. err message is:%s", msgID, "正常");
                ...
                if (msgID == IDUM_UseItem) {
                    CCNode* jinhe = getChildByTag(TAG_LAYER_SHOP)->getChildByTag(3);
                    if (!jinhe) {
            ...
            case SERR_Exception:
                CCLog("error id is:%d. err message is:%s", msgID, "服务器异常");
                break;
                
            case SERR_Msg:
                CCLog("error id is:%d. err message is:%s", msgID, "error msg");
                break;
                
            case SERR_Notplt:
                CCLog("error id is:%d. err message is:%s", msgID, "no data template");
                break;
                
            case SERR_Illegal:
                CCLog("error id is:%d. err message is:%s", msgID, "非法的");
                break;
                
            case SERR_Arg:
                CCLog("error id is:%d. err message is:%s", msgID, "参数错误");
                break;
                
            case SERR_State:
                CCLog("error id is:%d. err message is:%s", msgID, "状态错误");
                break;
                
            case SERR_Remote:
                CCLog("error id is:%d. err message is:%s", msgID, "远端异常");
                break;
                
            case SERR_Nocopper:
                CCLog("error id is:%d. err message is:%s", msgID, "铜币不足");
                break;
                
            case SERR_Nogold:
                CCLog("error id is:%d. err message is:%s", msgID, "元宝不足");
                break;
                
            case SERR_Noticket:
                CCLog("error id is:%d. err message is:%s", msgID, "入场券不足");
                break;
                
            case SERR_Nomat:
                CCLog("error id is:%d. err message is:%s", msgID, "材料不足");
                break;
                
            case SERR_Norole:
                CCLog("error id is:%d. err message is:%s", msgID, "没有指定的角色");
                break;
                
            case SERR_Notfriend:
                CCLog("error id is:%d. err message is:%s", msgID, "不是好友");
                break;
                
            case SERR_Friendyet:
                CCLog("error id is:%d. err message is:%s", msgID, "已经是好友");
                break;
                
            case SERR_HasInvite:
                CCLog("error id is:%d. err message is:%s", msgID, "已经邀请");
                break;
                
            case SERR_Blackyet:
                CCLog("error id is:%d. err message is:%s", msgID, "已经拉黑");
                break;
...
void MainScene::respShopList(const WebSocket::Data &data)
{
    UM_Shop resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
    {
        ...
        for( int i = 0; i < num; i++)
        {
            ...
            CCLog("shop item id is:%d", item.tpltid());
...
void MainScene::respHeroUp(const WebSocket::Data &data)
{
    UM_Hero resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
void MainScene::respEquipUpdate(const WebSocket::Data &data)
{
    UM_EquipUpdate resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
// item buy or use
void MainScene::respItemUpdate(const WebSocket::Data &data)
{
...
void MainScene::respRoleInfo(const WebSocket::Data &data)
{
    UM_RoleInfo resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
    {
        ...
        info.freeTicket = userInfo.free_ticket();
        info.fefreshTime = userInfo.refresh_time();
        //CCLog("user id:%d, name is:%s, sex is:%d, createTime is:%d, level is:%d", info.roleID, info.roleName.c_str(), info.sex, info.createTime, info.level);
        
        //CCLog("duanwei is :%d, stars is:%d", info.duanwei, info.stars);
        if(info.roleName == "")
        {
            char temp[128] = {0};
            sprintf(temp, "S%d", info.roleID);
...
void MainScene::respFans(const WebSocket::Data &data)
{
...
void MainScene::respRanks(const WebSocket::Data &data)
{
...
void MainScene::respFightLikes(const WebSocket::Data &data)
{
    UM_FightLikes resp;
     bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
...
void MainScene::respHotMsg(const WebSocket::Data &data)
{
    UserInfo* ulayer = (UserInfo*)getChildByTag(TAG_LAYER_USERINFO);
    if(ulayer)
...
void MainScene::respMsgList(const WebSocket::Data &data)
{
    UserInfo* ulayer = (UserInfo*)getChildByTag(TAG_LAYER_USERINFO);
    if(ulayer)
...
void MainScene::respFriendList(const WebSocket::Data &data)
{
    if(data.len <= 2)
        return;
...
void MainScene::respAddFriend(const WebSocket::Data &data)
{
    UM_AddFriend resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
void MainScene::respAddInvite(const WebSocket::Data &data)
{
    UM_AddInvite resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
// 提示用户被拒绝
void MainScene::respDelInvite(const WebSocket::Data &data)
{
    UM_DelInvite resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
void MainScene::respSeasonRank(const WebSocket::Data &data)
{
    UserInfo* ulayer = (UserInfo*)getChildByTag(TAG_LAYER_USERINFO);
    if(ulayer)
...
void MainScene::respGameRecord(const WebSocket::Data &data)
{
    UserInfo* ulayer = (UserInfo*)getChildByTag(TAG_LAYER_USERINFO);
    if(ulayer)
...
void MainScene::respPhotos(const WebSocket::Data &data)
{
    for (int i = 2; i < data.len; i++) {
//        printf("%d,", (uint8_t)data.bytes[i]);
    }
...
void MainScene::respIcons(const WebSocket::Data &data)
{
    UM_Icons resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
void MainScene::respAwardList(const WebSocket::Data &data)
{
    UM_AwardList resp;
    bool re = resp.ParseFromArray(data.bytes + 2, data.len - 2);
    if(re)
...
bool ZhanDouEnd::init()
{
    ...
    CCSprite* dikuang = CCSprite::create("zhaopian-03.png");
    dikuang->setScale(0.77f);
    dikuang->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+280));
    addChild(dikuang);
    
    dikuang = CCSprite::create("benlunjieshu-01.png");
    dikuang->setScale(0.6f);
    dikuang->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+280));
    addChild(dikuang);
    
    Button* lef = Button::createBtnWithSpriteFrameName("jiesuan-btn-2.png", false);
    lef->setPosition(ccp(winSize.width*0.5f - 275, winSize.height * 0.5f+210));
    lef->setScale(0.6f);
    ...
    lef->setPosition(ccp(winSize.width*0.5f + 270, winSize.height * 0.5f+210));
    lef->setScale(0.6f);
    ...
    lef->setPosition(ccp(winSize.width*0.5f , 60));
...
void ZhanDouEnd::InitRank()
{
    ...
    if (mCurTouch == 1) {
        ...
        lab->setPosition(ccp(winSize.width*0.5f - 500, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
    }
        CCLabelTTF* lab = CCLabelTTF::create(Global::getInstance()->getString("93"), "STXingkai.ttf", 25);
        lab->setPosition(ccp(winSize.width*0.5f - 320, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
        
        lab = CCLabelTTF::create(Global::getInstance()->getString("94"), "STXingkai.ttf", 25);
        lab->setPosition(ccp(winSize.width*0.5f -130, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
    if (mCurTouch == 1) {
        lab = CCLabelTTF::create(Global::getInstance()->getString("95"), "STXingkai.ttf", 25);
        lab->setPosition(ccp(winSize.width*0.5f + 30, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
        
        lab = CCLabelTTF::create(Global::getInstance()->getString("96"), "STXingkai.ttf", 25);
        lab->setPosition(ccp(winSize.width*0.5f + 140, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
        
        lab = CCLabelTTF::create(Global::getInstance()->getString("97"), "STXingkai.ttf", 25);
        lab->setPosition(ccp(winSize.width*0.5f + 250, winSize.height * 0.5f+150));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab);
    //if (mCurTouch == 1) {
        CCSprite* dikuang = CCSprite::create("zhaopian-03.png");
        dikuang->setScale(0.77f);
        dikuang->setPosition(ccp(winSize.width * 0.5f, 160));
        ...
        lab->setPosition(ccp(70,160));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab,1);
        
        //table_bg = CCSprite::create("touxiang002.png");
        //table_bg->setScale(0.2f);
        //table_bg->setPosition(ccp(160,35));
        ...
        lab->setPosition(ccp(200,160+16));
        ...
        lab->setPosition(ccp(200,160-15));
        lab->setAnchorPoint(ccp(0.0f,0.5f));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab,1);
        ...
        lab->setPosition(ccp(600,160));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab,1);
        
        sprintf(labStr, "%d",info.eat);
        lab = CCLabelTTF::create(labStr, "STXingkai.ttf", 30);
        lab->setPosition(ccp(700,160));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab,1);
        
        sprintf(labStr, "%d",info.copper);
        lab = CCLabelTTF::create(labStr, "STXingkai.ttf", 30);
        lab->setPosition(ccp(800,160));
        lab->setColor(ccc3(0, 0, 0));
        layer->addChild(lab,1);
    }else if (mCurTouch == 2)
    {
...
void ZhanDouEnd::ReturnBegan(CCObject* pSender)
{
...
void ZhanDouEnd::clickLeft(CCObject* pSender)
{
    resetBtnState();
...
void ZhanDouEnd::clickRight(CCObject* pSender)
{
    resetBtnState();
    Button* shouye = (Button*)pSender;
    shouye->setPressIconShow(true);
...
void ZhanDouEnd::tableCellTouched(CCTableView* table, CCTableViewCell* cell)
{
    mTouchDex = cell->getIdx();
}
CCTableViewCell* ZhanDouEnd::tableCellAtIndex(CCTableView* table, unsigned int idex)
{
...
CCSize ZhanDouEnd::cellSizeForTable(CCTableView* table)
{
    return CCSizeMake(winSize.width, 70);
}
unsigned int ZhanDouEnd::numberOfCellsInTableView(CCTableView* table)
{
    if (mCurTouch == 1) {
...
CCSize ZhanDouEnd::getTableSize()
{
    return CCSizeMake(winSize.width, 250);
}
void ZhanDouEnd::createTableViewCell(CCNode* cell, CCTableView* table, int idex)
{
    if (mCurTouch == 1) {
        EndGame info = mRankVec[idex];
        
        CCSprite* table_bg = CCSprite::create("xuanxiangkuang.png");
        table_bg->setScale(0.65f);
        table_bg->setPosition(ccp(winSize.width/2,35));
        cell->addChild(table_bg,1);
        ...
        if(info.icon < 101)
        {
            CCTexture2D* tex = ExchangeInfo::GetTextureForData(info.iconStr.c_str());
        ...
        iconCir->setPosition(ccp(160, 35));
        cell->addChild(iconCir, 1);
        
        NameColor color = Global::getInstance()->GetNameColor(info.name);
        lab = CCLabelTTF::create(info.name.c_str(), "STXingkai.ttf", 25);
        lab->setPosition(ccp(200,51));
        lab->setAnchorPoint(ccp(0.0f,0.5f));
        lab->setColor(ccc3(color.colorR, color.colorG, color.colorB));
        cell->addChild(lab,1);
        ...
        lab = CCLabelTTF::create(temp, "STXingkai.ttf", 30);
        lab->setPosition(ccp(600,35));
        lab->setColor(ccc3(0, 0, 0));
        cell->addChild(lab,1);
        
        sprintf(labStr, "%d",info.eat);
        lab = CCLabelTTF::create(labStr, "STXingkai.ttf", 30);
        lab->setPosition(ccp(700,35));
        lab->setColor(ccc3(0, 0, 0));
        cell->addChild(lab,1);
        
        sprintf(labStr, "%d",info.copper);
        lab = CCLabelTTF::create(labStr, "STXingkai.ttf", 30);
        lab->setPosition(ccp(800,35));
        lab->setColor(ccc3(0, 0, 0));
        cell->addChild(lab,1);
...
void ZhanDouEnd::clickGuanzhu(CCObject* pSender)
{
...
void ZhanDouEnd::clickXihuan(CCObject* pSender)
{
    Button* btn = (Button*)pSender;
    CCTableViewCell* cell = (CCTableViewCell*)btn->getParent();
    int idex = cell->getIdx();
...
bool AddFriend::init()
{
    ...
    CCSprite* bg = CCSprite::create("yaoqing_bg_2.png");
    bg->setPosition(ccp(winSize.width-202*0.6f+10, 200));
    bg->setScale(0.6f);
    addChild(bg);
    
    CCLabelTTF* lab = CCLabelTTF::create(Global::getInstance()->getString("105"), "STXingkai.ttf", 25);
    lab->setPosition(ccp(winSize.width-202*0.6f+10, 200));
    addChild(lab,1);
    
    bg = CCSprite::create("yaoqing_bg_1.png");
    bg->setPosition(ccp(winSize.width-202*0.6f, 150));
    bg->setScale(0.6f);
    addChild(bg);
    
    Button* lef = Button::createBtnWithSpriteFrameName("yaoqing_btn.png", false);
    lef->setPosition(ccp(winSize.width-202*0.6f-92*0.6f +10, 95));
    //lef->setPressIcon("fanhuidating-01.png", false);
    lef->setScale(0.6f);
    lef->setOnClickCallback(callfuncO_selector(AddFriend::clickOK), this);
    addChild(lef);
    
    bg = CCSprite::create("xuanze-01.png");
    bg->setPosition(ccp(winSize.width-202*0.6f-92*0.6f+10 , 95));
    bg->setScale(0.6f);
    addChild(bg);
    ...
    bg = CCSprite::create("xuanze-02.png");
    bg->setPosition(ccp(winSize.width-202*0.6f+92*0.6f +10, 95));
    bg->setScale(0.6f);
    addChild(bg);
...
void AddFriend::clickOK(CCObject* pSender)
{
    MainScene* scene = (MainScene*)getParent();
...
void AddFriend::clickCancel(CCObject* pSender)
{
    MainScene* scene = (MainScene*)getParent();
...
bool UserLow::init()
{
    ...
    CCSprite* spBg = CCSprite::create("user_low_bg.png");
    spBg->setPosition(ccp(winSize.width/2,winSize.height/2));
    spBg->setScale(0.6f);
    addChild(spBg,0,101);
    
    spBg = CCSprite::create("low_line.png");
    spBg->setPosition(ccp(winSize.width/2,winSize.height/2+200));
    spBg->setScale(0.6f);
    addChild(spBg);
    ...
    lab->setColor(ccc3(0, 0, 0));
    lab->setAnchorPoint(ccp(0.0f, 0.5f));
    lab->setPosition(ccp(winSize.width * 0.5f-110, winSize.height * 0.5f+230));
    addChild(lab, 2);
    
    
    CCSprite* youxi = CCSprite::create(duan.icon.c_str());
    youxi->setPosition(ccp(winSize.width * 0.5f-25, winSize.height * 0.5f+230));
    ...
    lab->setColor(ccc3(0, 0, 0));
    lab->setAnchorPoint(ccp(0.0f, 0.5f));
    lab->setPosition(ccp(winSize.width * 0.5f-10, winSize.height * 0.5f+230));
    addChild(lab, 2);
    ...
    if (info.stars > 3) {
        ...
        lab->setColor(ccc3(0, 0, 0));
        lab->setAnchorPoint(ccp(0.0f,0.5f));
        lab->setPosition(ccp(winSize.width * 0.5f+60, winSize.height * 0.5f+230));
        addChild(lab, 2);
        
        CCSprite* xing = CCSprite::create("hecheng-xx-02.png");
        xing->setScale(0.6f);
        xing->setPosition(ccp(winSize.width * 0.5f+100, winSize.height * 0.5f+230));
        addChild(xing, 2);
    }else{
        for(int i = 0;i<3;i++)
        {
            if (info.stars > i) {
                CCSprite* xing = CCSprite::create("hecheng-xx-02.png");
                xing->setScale(0.6f);
                xing->setPosition(ccp(winSize.width * 0.5f+60 + i*30, winSize.height * 0.5f+230));
                addChild(xing, 2);
            }else{
                CCSprite* xing = CCSprite::create("hecheng-xx-01.png");
                xing->setScale(0.6f);
    ...
    headbg->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+90));
    ...
    CCSprite* head = CCSprite::create(temp);
    head->setScale(0.5f);
    head->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+90));
    addChild(head, 2);
    
    lab = CCLabelTTF::create(info.roleName.c_str(), "STXingkai.ttf",35);
    lab->setColor(ccc3(0, 0, 0));
    lab->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f-20));
    addChild(lab, 2);
    
    std::map<int, std::string> hInfo = ExchangeInfo::getProvinces();
    lab = CCLabelTTF::create(hInfo[info.province].c_str(), "STXingkai.ttf", 35);
    lab->setPosition(ccp(winSize.width * 0.5f, winSize.height*0.5f - 100));
    lab->setColor(ccc3(0, 0, 0));
    addChild(lab, 2);
    
    spBg = CCSprite::create("low_line.png");
    spBg->setPosition(ccp(winSize.width/2,winSize.height/2-150));
    spBg->setScale(0.6f);
    addChild(spBg);
...
void UserLow::InitLike(EndGame info)
{
    ...
    lef->setPosition(ccp(winSize.width * 0.5f+70, winSize.height*0.5f - 200));
...
void UserLow::clickXihuan(CCObject* pSender)
{
...
void UserLow::clickGuanzhu(CCObject* pSender)
{
    MainScene* scene = (MainScene*)getParent()->getParent();
...
bool UserLow::ccTouchBegan(CCTouch* pTouch,CCEvent* pEvent)
{
...
void UserLow::ccTouchEnded(CCTouch* pTouch,CCEvent* pEvent)
{
    if(isTouchIn)
        return;
    //CCTouch* pTouch = (CCTouch*)pTouches->anyObject();
...
bool ReqOpenFight::init()
{
    ...
    lab->setColor(ccc3(0, 0, 0));
    lab->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+50));
    addChild(lab, 2);
    
    lab = CCLabelTTF::create(Global::getInstance()->getString("108"), "STXingkai.ttf",30);
    lab->setColor(ccc3(0, 0, 0));
    lab->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f+10));
    addChild(lab, 2);
    
    Button* headbg = Button::createBtnWithSpriteFrameName("huifubisai-01.png", false);
    headbg->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f-70));
...
void ReqOpenFight::clickOK(CCObject* pSender)
{
    MainScene* scene = (MainScene*)getParent();
...
bool ReqOpenFight::ccTouchBegan(CCTouch* pTouch,CCEvent* pEvent)
{
    //CCTouch* pTouch = (CCTouch*)pTouches->anyObject();
    CCNode* node = getChildByTag(101);
    CCPoint pt = node->convertToNodeSpace(CCDirector::sharedDirector()->convertToGL(pTouch->getLocationInView()));
    CCRect rt = CCRectMake(0, 0, node->getContentSize().width*0.3f, node->getContentSize().height*0.4f);
    
    isTouchIn = rt.containsPoint(pt);
    
    return true;
}
void ReqOpenFight::ccTouchEnded(CCTouch* pTouch,CCEvent* pEvent)
{
    if(isTouchIn)
        return;
    //CCTouch* pTouch = (CCTouch*)pTouches->anyObject();
