...
const std::string SqLiteDataProvider::CFGPARAM_SQLITE_DB_DEF = "ants.db";
...
#ifdef USE_SQLITE_ENCRYPT
	sqlite3_key(pFile, "qazqwe123", 9);
#endif
	if(rc == SQLITE_OK)  	
	{  		
...
SqLiteDataProvider::connect()
{
	...
	CCLog("Trying to connect with SQLite database");

    // sqlite3_open creates the database file if it does not exist as a side-effect.	
    std::string path = Global::getDataPath(dbName);
	CCLog("db path= %s",path.c_str());
	
	if ((ret = sqlite3_open(/*":memory:"*/path.c_str(), &mDb)) != SQLITE_OK) {
		...
		CCLog("Connection to database failed!(%d)", ret);
    ...
    mIsConnected = true;
    CCLog("Connection to database successful.");
}
